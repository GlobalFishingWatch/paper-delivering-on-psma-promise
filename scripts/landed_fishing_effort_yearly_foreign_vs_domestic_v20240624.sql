## Keywords: Landed fishing effort in kwh by foreign vs domestic
...
CREATE
OR REPLACE
TABLE `world-fishing-827.scratch_jaeyoon.landed_fishing_effort_yearly_foreign_vs_domestic_v20240624` AS
...
WITH
  ...
  -----------------------------------------------------------
  -- Target vessels either fishing vessels or carrier vessels
  -----------------------------------------------------------
  target_vessels AS (
    SELECT DISTINCT
      ssvid,
  ...
  target_vessel_landings AS (
    ...
    FROM (
      ...
      FROM include_only_fishing_vessels
      UNION DISTINCT
      SELECT DISTINCT
  ...
  fishing_events AS (
    SELECT
      ...
      IF ((best_vessel_class = "squid_jigger" AND night_loitering > 0.5)
        ...
        hours * best_engine_power_kw, 0) AS fishing_hours
    FROM (
      SELECT
  ...
  -------------------------------------------------------------
  -- Group landed fishing effort by foreign vs domestic
  -------------------------------------------------------------
  sumup AS (
    SELECT
      timeline,
