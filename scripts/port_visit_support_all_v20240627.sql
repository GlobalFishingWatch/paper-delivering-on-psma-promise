...
create temp function minimum() as (timestamp('2021-01-01'));
...
--------------------------------------------
-- port visits by support vessels
--------------------------------------------
-- add reset count
...
all_events_landed_fishing as (
    select
        *,
        lag(n_all_fishing) over (
            partition by vessel_id
            order by event_start asc
        ) as x
    from all_events_all_fishing
),


encounter_with_all_fishing as (
    select *
...
------------------------
-- get encounter events by support vessels meeting fishing vessels
all_fishing_encouter_support as (
    select
    ...
    where event_id in (select event_id from encounter_with_all_fishing)
),


all_events_support as (
    select * 
    from all_fishing_encouter_support
    union all
    select *
    from port_visit_event_clean
    where vessel_id in (select vessel_id from all_fishing_encouter_support)
),


-- add reset count
all_events_support_reset_count as (
    select
        vessel_id,
        event_id,
        event_start,
    ...
    from all_events_support
),


-- cumulative sum with reset logic
all_events_support_all_fishing as (
    select
        *,
        sum(case when event_type = 'encounter' then 1 else 0 end) over (
            partition by vessel_id, reset_count
            order by event_start asc
            rows between unbounded preceding and current row
        ) as n_all_fishing
    from all_events_support_reset_count
),


all_events_support_landed_fishing as (
    select
        *,
        lag(n_all_fishing) over (
            partition by vessel_id
    ...
    from all_events_support_all_fishing
),


port_visit_support as (
    select *
    from all_events_support_landed_fishing
...
port_visit_support_info as (
    ...
    using(ssvid, year)
    where event_id in (select event_id from port_visit_support)
)
