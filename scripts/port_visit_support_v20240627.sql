...
encounter_with_high_seas_fishing as (
    select *
...
high_sea_fishing_encouter_support as (
    select
    ...
    where event_id in (select event_id from encounter_with_high_seas_fishing)
),


all_events_support as (
    select * 
    from high_sea_fishing_encouter_support
    union all
    select *
    from port_visit_event_clean
    where vessel_id in (select vessel_id from high_sea_fishing_encouter_support)
),
...
-- cumulative sum with reset logic
all_events_support_high_seas_fishing as (
    select
        *,
...
all_events_support_landed_fishing as (
    ...
    from all_events_support_high_seas_fishing
),
